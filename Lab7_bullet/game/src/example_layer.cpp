...

{
	...
	float cow_scale = 1.f / glm::max(cow_model->size().x, glm::max(cow_model->size().y,
		cow_model->size().z));
	cow_props.position = { 0.f,2.f, 0.f };
	...
	m_cow->set_angular_factor({0,0,0});
	m_cow_box.set_box(cow_props.bounding_shape.x * 2.f * cow_scale,
		...
		* cow_scale, cow_props.position - glm::vec3(0.f, m_cow->offset().y, 0.f) *
		m_cow->scale());
	...
	sphere_props.position = { 0.f, 5.f, 0.f };
