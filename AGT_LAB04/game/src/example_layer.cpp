...

{
	...
	//Mannequin model for the player
	...
	{ engine::texture_2d::create("assets/textures/random_texture.png",false) };
	...
	jeep_props.position = { 0.f,0.5f, 1.5f };
	jeep_props.scale = glm::vec3(jeep_scale);
	jeep_props.bounding_shape = jeep_model->size() / 2.f * cow_scale;
...
void example_layer::on_render() 
{
	...
	glm::mat4 jeep_transform(1.f);
	jeep_transform = glm::translate(jeep_transform, m_jeep->position());
	jeep_transform = glm::rotate(jeep_transform,-3.141592f/2.f, glm::vec3(1,0,0));
	jeep_transform = glm::scale(jeep_transform, m_jeep->scale()*.125f);
