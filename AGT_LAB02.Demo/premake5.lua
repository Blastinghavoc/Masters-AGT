...
project "game"
	...
	postbuildcommands {
		--"{COPY} $(SolutionDir)%{LibDir.fmod}fmod.dll $(ProjectDir)$(OutDir)\"
		--'xcopy /y /d "$(SolutionDir)engine\external\FMOD\lib\x64\fmod.dll" "$(ProjectDir)$(OutDir)"'
		"{COPY} $(SolutionDir)%{LibDir.fmod}fmod.dll %{cfg.targetdir}"
	}
