...
bool engine::spatialised_sound::load(const std::string& filePath)
{
	...
	//This was resetting the global settings each time!
	/*result = engine::audio_manager::system()->set3DSettings(1.0, 0.5, 1.0);*/

	return result == FMOD_OK;
}

bool engine::spatialised_sound::play(glm::vec3 position, float min_distance)
{
	...
	m_channel->set3DMinMaxDistance(min_distance, 50.0);
