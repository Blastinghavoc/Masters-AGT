...
void trigger_box::get(glm::vec3 &bottom, float &width, float &height, float &depth)
{
	bottom = m_bottom;
	width = m_width;
	height = m_height;
...
void trigger_box::set_box(float width, float height, float depth, glm::vec3 position)
{
	m_width = width;
	m_height = height;
	m_depth = depth;

	m_bottom = position - glm::vec3(m_width / 2.f, m_height / 2.f, m_depth / 2.f);
...
void trigger_box::on_update(glm::vec3 position)
{
	m_bottom = position - glm::vec3(m_width/2.f, m_height/2.f, m_depth/2.f);
}

void trigger_box::on_render(const engine::ref<engine::shader>& shader,float r, float g, float b)
{
	glm::mat4 transform(1.0f);
	transform = glm::translate(transform, m_bottom + glm::vec3(0.f, m_height / 2.f, 0.f));
...
bool trigger_box::collision(trigger_box& other_box)
{
