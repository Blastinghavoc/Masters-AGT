...
void bounding_box::get(glm::vec3 & position, float &width, float &height, float &depth)
{
	position = m_position;
	width = m_width;
	height = m_height;
...
void bounding_box::set_box(float width, float height, float depth, glm::vec3 position)
{
	m_width = width;
	m_height = height;
	m_depth = depth;
...
void bounding_box::on_update(glm::vec3 position, float rotation, glm::vec3 axis)
{
	m_position = position;
...
void bounding_box::on_render(float r, float g, float b, const engine::ref<engine::shader>& shader)
{
